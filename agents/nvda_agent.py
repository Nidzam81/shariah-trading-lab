"""
NVDA Intraday Mean-Reversion Trading Agent
============================================
Strategy: RSI(14) oversold dip + SMA 200 trend filter on 5-min candles
Asset: NVDA (US Market)
...
STATE_DIR = os.path.join(os.path.expanduser("~"), "AppData", "Local", "hermes", "nvda_agent")
STATE_FILE = os.path.join(STATE_DIR, "state.json")
LOG_FILE = os.path.join(STATE_DIR, "trading_log.jsonl")
...
def upload_trade(side, price, qty, reason, entry_price=0, pnl=0):
    ...
    try:
        ...
        upload_trade_log_for_agent("nvda", side, price, qty, reason, entry_price, pnl)
    except Exception:
        pass
...
def fetch_data():
    ...
    return closes, closes[-1], calc_sma(closes, SMA_PERIOD), calc_rsi(closes, RSI_PERIOD)
...
def main():
    ...
    if position_open and entry_price > 0:
        ...
        if price is None:
            log_event({"type": "exit_check", "status": "no_price", "msg": "No price, skipping exit check"})
        elif price >= tp:
            log_event({"type": "exit_signal", "reason": "TP", "entry": ep, "current": price,
                        "msg": f"TP hit! Entry={ep:.2f} Current={price:.2f} (+{(price/ep-1)*100:.2f}%)"})
            ...
            log_event({"type": "exit_signal", "reason": "SL", "entry": ep, "current": price,
                        "msg": f"SL hit! Entry={ep:.2f} Current={price:.2f} ({(price/ep-1)*100:.2f}%)"})
            ok, res = place_order("SELL", state.get("qty", QTY))
            if ok:
                pnl = (price - ep) * state.get("qty", QTY)
                save_state({"position_open": False, "entry_price": 0.0, "qty": 0, "order_id": None})
                log_event({"type": "order_filled", "side": "SELL", "reason": "SL", "order_id": res})
                send_telegram(f"NVDA STOP LOSS (SIMULATE)\nSELL {state.get('qty',QTY)} @ ${price:.2f}\nEntry: ${ep:.2f} ({(price/ep-1)*100:.2f}%)\nOrder: {res}")
            ...
            log_event({"type": "position_monitor", "entry": ep, "current": price, "tp": tp, "sl": sl,
                        "pnl_pct": pct, "msg": f"Holding NVDA: entry={ep:.2f} current={price} TP={tp:.2f} SL={sl:.2f} PnL={pct}%"})
        return

    # ENTRY: No position
    if not in_window:
        log_event({"type": "skip", "reason": "outside_window", "msg": "Outside 9:30-15:30 EST entry window"})
        return

    closes, current_price, sma200, rsi14 = fetch_data()
    if current_price is None or sma200 is None or rsi14 is None:
        log_event({"type": "data_fail", "msg": "Failed to fetch NVDA data"})
        return

    log_event({"type": "market_data", "price": current_price, "sma200": sma200, "rsi14": rsi14,
                "msg": f"NVDA: price={current_price:.2f} SMA200={sma200:.2f} RSI14={rsi14}"})
    ...
    if rsi14 <= RSI_BUY_THRESHOLD:
        log_event({"type": "buy_signal", "rsi": rsi14, "price": current_price,
                    "msg": f"RSI oversold! RSI={rsi14} <= {RSI_BUY_THRESHOLD} price={current_price:.2f} BUYING {QTY}"})
        ...
        if ok:
            ...
            log_event({"type": "order_filled", "side": "BUY", "price": current_price, "qty": QTY, "order_id": res,
                        "msg": f"BUY {QTY} NVDA @ {current_price:.2f} order={res} TP={tp:.2f} SL={sl:.2f}"})
            send_telegram(f"NVDA BUY SIGNAL (SIMULATE)\nBUY {QTY} @ ${current_price:.2f}\nRSI: {rsi14} SMA200: ${sma200:.2f}\nTP: ${tp:.2f} SL: ${sl:.2f}\nOrder: {res}")
